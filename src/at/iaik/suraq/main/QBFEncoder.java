...
public class QBFEncoder {
    ...
    // List<PropositionalVariable> controlSignals = logicParser.getControlVariables()
            ...
            Set<PropositionalVariable> tseitinEncoding)
    {
        ...
        if(!checkFormulaReady(formula))
            throw new RuntimeException("Formula not ready for QBF Encoding (AND).");
        AndFormula top = (AndFormula) formula;
        
        for(Formula innerFormula : top.getConjuncts())
        {
            if(!(innerFormula instanceof OrFormula))
                throw new RuntimeException("Formula not ready for QBF Encoding (OR).");
            ...
            for(Formula disjunctElem : inner.getDisjuncts())
            {
                ...
                Formula var = (disjunctElem instanceof NotFormula)
                                        ? ((NotFormula)disjunctElem).getNegatedFormula()
                                        : disjunctElem;
                if(var == null || !(var instanceof PropositionalVariable))
                    throw new RuntimeException("Formula not ready for QBF Encoding (PropVar/Not).");
        ...
        StringBuilder header = makeQuantors(formula, noDependenceVars, controlSignals, tseitinEncoding);
            ...
            Set<PropositionalVariable> tseitinEncoding)
    {
        ...
        // FIXME: Assumption: ( A inputs )
        // inputs is everything that is not control, no_dependence, tseitin
        // inputs can't be read out of the .smt2 file, right?
        ...
        // input vars
        Set<PropositionalVariable> inputVars = formula.getPropositionalVariables();
    ...
    public boolean checkFormulaReady(Formula formula)
    {
        ...
        {
            System.err.println("QBFEncoder: There are DomainVariables in the Formula!");
            return false;
        }
        if(formula.getFunctionMacros().size() != 0)
        {
            System.err.println("QBFEncoder: There are FunctionMacros in the Formula!");
            return false;
        }
        if(formula.getUninterpretedFunctions().size() != 0) 
        {
            System.err.println("QBFEncoder: There are UninterpretedFunctions in the Formula!");
            return false;
        }
